...
-- Setup file for defining and loading banking data (note that this isn't
-- exactly the same dataset as the one used in lectures)
-- Also used in A3, with some DDL documentation added for reference. For
-- students reviewing this code, note that this A2 DDL is simplified
-- without some constraints that might make sense to support.
...
-- Represents current bank accounts unqiuely identified by an account_number
-- No account can belong to more than one branch.
-- Requires all non-null values.
CREATE TABLE account (
    -- May include letters, e.g. 'A-100'
    account_number  VARCHAR(15)    NOT NULL,
    branch_name     VARCHAR(15)    NOT NULL,
    -- current balance in USD, no more than $9,999,999,999.99
    balance         NUMERIC(12, 2) NOT NULL,
    PRIMARY KEY (account_number)
);

-- Represents a banking branch located in a city and managed assets
-- Requires all non-null values.
CREATE TABLE branch (
    branch_name    VARCHAR(15)    NOT NULL,
    branch_city    VARCHAR(15)    NOT NULL,
    -- current assets in USD, no more than $999,999,999,999.99
    assets         NUMERIC(14, 2) NOT NULL,
    PRIMARY KEY (branch_name)
);

-- Represents a customer, uniquely identified by their name (clever...)
-- and having an associated street address/city (other location info ignored).
-- Requires all non-null values.
CREATE TABLE customer (
    customer_name    VARCHAR(15) NOT NULL,
    -- You can't be our customer if you live in a wordy street/city...
    -- characters cost $$$
...
-- Represents a loan at a branch, uniquely identified by a loan number.
-- Requires all non-null values.
CREATE TABLE loan (
    -- May include letters, e.g. 'L-121'
    loan_number  VARCHAR(15)    NOT NULL,
    branch_name  VARCHAR(15)    NOT NULL,
    -- current loan amount in USD, no more than $9,999,999,999.99
    amount       NUMERIC(12, 2) NOT NULL,
    PRIMARY KEY (loan_number)
);

-- A referencing relation to model a depositor for an account, uniquely 
-- identified by the (customer_name, account_number).
-- Requires all non-null values.
CREATE TABLE depositor (
    customer_name   VARCHAR(15) NOT NULL,
    account_number  VARCHAR(15) NOT NULL,
    -- Multiple customers can have more than one account, vice-versa
    PRIMARY KEY (customer_name, account_number),
    FOREIGN KEY (account_number) REFERENCES account(account_number),
    FOREIGN KEY (customer_name) REFERENCES customer(customer_name)
);

-- A referencing relation to model a borrower for a loan, uniquely 
-- identified by the (customer_name, loan_number).
-- Requires all non-null values.
CREATE TABLE borrower (
    customer_name  VARCHAR(15) NOT NULL,
    loan_number    VARCHAR(15) NOT NULL,
    -- Multiple customers can have more than one loan, vice-versa
